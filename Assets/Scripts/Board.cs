...
public class Board : MonoBehaviour
{
    ...
    // the Node representing the quiz of the maze
    Node m_quizNode;
    public Node quizNode { get { return m_quizNode; } }
    ...
    public float drawQuizTime = 2f;
    public float drawQuizDelay = 2f;
    public iTween.EaseType drawQuizEaseType = iTween.EaseType.easeOutExpo;
    ...
    Node FindQuizNode()
    {
        return m_allNodes.Find(n => n.isLevelQuiz);
    ...
    {
        if (quizPrefab != null && m_quizNode != null)
        {
            GameObject quizInstance = Instantiate(quizPrefab, m_quizNode.transform.position,
                                                  Quaternion.identity);
            quizPrefab.gameObject.tag = "questionText";
            iTween.ScaleFrom(quizInstance, iTween.Hash(
                "scale", Vector3.zero,
                "time", drawQuizTime,
                "delay", drawQuizDelay,
                "easetype", drawQuizEaseType
            ));
        }
    }
